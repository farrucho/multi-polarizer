  ...
  public:
    ...
    void configuring() {
      ...
      for (int j = 1; j < 6; ++j)
      {
        ...
        if(expr.param[j] >= 0 && expr.param[j] <= 900){
            stepperArray[j-1].nsteps(expr.param[j],dirToTop);
            // polarizer j rodado
        }else{
    ...
    //   // // // comment esta linha if (DEBUG) Serial.println("1_started");
      ...
      if(expr.param[6] > 0 && expr.param[6] < 6){
        int initialSteps = expr.param[(int)expr.param[6]]; // step inicial do varrimento, index dos polarzs. sao 1-5 do expr.parm
        for(int currentStep = initialSteps; currentStep < expr.param[7];currentStep=currentStep+1){
          stepperArray[expr.param[6]-1].step(dirToLow);
          delay(4);
          //  #1   |   348º   |    503mv    |    
          Serial.print("\n");
